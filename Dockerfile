...
ENV APP_SOURCE_DIR=/usr/local/src/basic-react-project \
...
RUN mkdir -p "/usr/local/src/basic-react-project/node_modules"; chown node "/usr/local/src/basic-react-project"; chown node "/usr/local/src/basic-react-project/node_modules"
...
COPY --chown=node ./.basic-react-project/yarnrc-docker.template /home/node/.yarnrc
